...
This is the project-level CLAUDE.md that ships with the Jetson harness. Copy it into the root of a project that adopts the harness. It governs Claude Code sessions running against that project. It is distinct from the repo-root CLAUDE.md of `harness-engineering` itself, which governs sessions building the harness.
...
Platform: NVIDIA Jetson AGX Orin, JetPack R36 (release 5.0), Ubuntu 22.04.5 LTS, aarch64.
...
Security tool versions: Semgrep 1.163.0 (conda base and system Python 3.10), gitleaks v8.21.2 (/usr/local/bin/), shellcheck 0.8.0 (apt), pre-commit 4.6.0 (conda base).

CUDA 12.6 is present on this device. The harness does not depend on CUDA. The harness does not manage GPU resources.
