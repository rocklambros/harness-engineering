# Post-Mac 10 — Build closeout: CHECKPOINT refresh, cross-doc consistency, behavioral verification protocol

## Operational preconditions (read before invoking)

Open a fresh Claude Code session. Run from `/Users/klambros/harness-engineering/` as the working directory. Operations 06 (README), 07 (USER_GUIDE), 08 (HARNESS_GUIDE), and 09 (JOURNEY) have all committed. This prompt closes out the Mac build.

This prompt does three small things: refreshes CHECKPOINT.md (gitignored, local only), runs a consistency check across the four documentation deliverables, and writes the behavioral verification runbook Rock executes separately in a fresh Claude Code session against a test project.

<role>
You are closing out the Mac build. The structural artifacts are committed. The documentation set is committed. What remains is three administrative deliverables: an internal state refresh, a consistency audit across four documents, and a runbook Rock follows to behaviorally verify the rebuilt `~/.claude/`.
...
<context_budget>Run /context at start and end. Phase reads the four documentation deliverables, CHECKPOINT, and the relevant phase outputs. Cache load is moderate. Record state in `phase-outputs/POST-MAC-10-CONTEXT.md`.</context_budget>

<parallel_tool_calls>
Initial parallel read: `CHECKPOINT.md`, `README.md`, `USER_GUIDE.md`, `HARNESS_GUIDE.md`, `JOURNEY.md`, `foundation/00-quality-contract.md`, `foundation/01-threat-model.md`, `mac/ARCHITECTURE.md`, `mac/harness/CLAUDE.md`, `mac/harness/settings.json`, `phase-outputs/PHASE-5-AUDIT.md`, `phase-outputs/POST-MAC-4-VERIFICATION.md`.
</parallel_tool_calls>

<scope>
Apply only to:
- `CHECKPOINT.md` (writes; gitignored, no commit)
- `phase-outputs/POST-MAC-10-CONTEXT.md` (writes)
- `phase-outputs/POST-MAC-10-NOTES.md` (writes: consistency findings and any fixes applied)
- `phase-outputs/POST-MAC-10-VERIFICATION-PROTOCOL.md` (writes: the runbook Rock executes)
- README.md, USER_GUIDE.md, HARNESS_GUIDE.md, JOURNEY.md (writes; only if the consistency check finds drift that needs fixing)
...
- **Documentation state.** Four public-facing documents committed: README.md (180-280 line front door per Operation 06), USER_GUIDE.md (1000-1800 line pragmatic day-to-day per Operation 07), HARNESS_GUIDE.md (1500-2500 line architectural reference per Operation 08), JOURNEY.md (1500-2500 line educational narrative per Operation 09). Repo is in its public-facing documented state. JOURNEY.ipynb retired in Operation 09.
...
Audit README, USER_GUIDE, HARNESS_GUIDE, and JOURNEY for internal consistency. Record findings in `phase-outputs/POST-MAC-10-NOTES.md` under §Consistency Audit. Any drift found gets fixed in the relevant document with a small targeted commit.

Specific consistency checks:

- **Links resolve.** Every link from any document to any other document, to foundation/, mac/, research/, or operations/ resolves to an existing file or section.
- **Mac state claim is consistent.** All four documents describe the Mac side as validated, rebuilt, operating. No document claims a state that does not match `phase-outputs/POST-MAC-4-VERIFICATION.md`.
- **Jetson and Windows state claim is consistent.** All four documents describe them as scaffolded with Mac cross-pollination applied. No document claims they are validated.
- **Quality Contract.** The five properties (QC.1 Security, QC.2 Tight code, QC.3 Comment the why, QC.4a Cache discipline, QC.4b Context window discipline, QC.5 Versioning) are described identically across README's brief mention, HARNESS_GUIDE's §6, foundation/00, and JOURNEY's epilogue. No drift in property names or counts. USER_GUIDE may reference them in passing; verify no contradiction.
- **Threat model.** The six threats (T1 prompt injection, T2 supply chain, T3 pre-trust initialization, T4 sub-command chain bypass, T5 cache poisoning, T6 hostile MCP server) are described identically across HARNESS_GUIDE's §7 and foundation/01. No drift.
...
- **USER_GUIDE / HARNESS_GUIDE split discipline.** USER_GUIDE describes operational behavior (what the user sees, what to type). HARNESS_GUIDE describes design rationale (why the design is this way). Spot-check three sections of each: any HARNESS_GUIDE section describing what the user sees belongs in USER_GUIDE; any USER_GUIDE section describing design rationale belongs in HARNESS_GUIDE.
- **Citation accuracy.** Spot-check 10 random citations across USER_GUIDE, HARNESS_GUIDE, and JOURNEY. Each cited path exists. Each cited section contains the claimed content.
...
- **Voice consistency.** README is pragmatic. USER_GUIDE is second-person operational. HARNESS_GUIDE is third-person architectural. JOURNEY is first-person retrospective. No drift between sections within a single document.
- **Mermaid block consistency.** Every mermaid block across the four documents parses cleanly (test via GitHub markdown preview or `mmdc` if available). Node labels for shared concepts are consistent across diagrams (e.g., the SessionStart hook is named identically in USER_GUIDE flowchart and HARNESS_GUIDE sequence; PreToolUse hooks are named identically across all diagrams that reference them). Diagrams have not drifted from the surrounding prose. No diagram contains decorative emoji, decorative color, or other ornamentation; mermaid is for information layout, not visual flair. No diagram exceeds 12 nodes (flowchart) or 8 lifelines (sequence). Where a diagram exceeds either cap, surface as a finding in NOTES; the fix is to split or convert to a table.
...
Write `phase-outputs/POST-MAC-10-VERIFICATION-PROTOCOL.md` as a self-contained runbook Rock executes in a separate fresh Claude Code session against any test project (NOT the harness-engineering repo). The protocol verifies that the rebuilt `~/.claude/` actually works as designed, not just that it parses on disk.
...
**Outcomes capture.** For each test, record in a new file Rock creates from this protocol's outputs (`phase-outputs/POST-MAC-10-VERIFICATION.md`):
...
Before Stage 1 claims "Mac is validated, rebuilt, documented, operating," verify Operations 04 (rebuild), 06 (README), 07 (USER_GUIDE), 08 (HARNESS_GUIDE), and 09 (JOURNEY) have all committed. The CHECKPOINT should not advertise a state that's not on disk.
...
- `phase-outputs/POST-MAC-10-CONTEXT.md`: context-budget record
- `phase-outputs/POST-MAC-10-NOTES.md`: consistency audit findings, fixes applied
- `phase-outputs/POST-MAC-10-VERIFICATION-PROTOCOL.md`: behavioral verification runbook
...
- `CHECKPOINT.md` reflects all five completed closeout operations (06-10).
- `phase-outputs/POST-MAC-10-VERIFICATION-PROTOCOL.md` exists and contains five tests with concrete commands.
...
```
docs: fix README link drift caught by Operation 10 consistency audit

Context: Operation 10 cross-document consistency check found <specific drift>.
...
Do not relitigate decisions during consistency checks. If USER_GUIDE and HARNESS_GUIDE describe a locked decision differently, the fix is to align them with the locked decision in CHECKPOINT and foundation, not to change the decision.
...
Do not modify locked-decision artifacts. CHECKPOINT updates the state section, not the locked decisions section. README, USER_GUIDE, HARNESS_GUIDE, and JOURNEY reflect locked decisions; they do not relitigate them.

If Stage 2 finds a class of drift that needs more than a small targeted fix (e.g., HARNESS_GUIDE's §4 describes a hook that does not exist in mac/harness/hooks/, or USER_GUIDE's behavior description does not match the hook code), do not paper over it. Surface as a finding in NOTES and ask Rock for direction. Documentation that describes a state not on disk is the worst kind of documentation drift.
...
After this operation completes, the original build sequence is done. Operations 06-10 were the documentation closeout; everything after is post-launch revision per the continuous operating model in CHECKPOINT.md.
